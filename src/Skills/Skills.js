...
export default function Skills({ sectionTitle = "Skills & Tools", skills = [] }) {
 ...
 return (
  <section id="skills" className="bg-slate-900 text-white py-20 px-4">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-center">
      {skills.map((group, index) => (
        <div
          ...
          className={`border ${group?.borderColor || 'border-blue-500'} rounded-xl p-6 cursor-pointer hover:shadow-lg transition`}
        >
          {/* Fixed group title */}
          <h3 className="text-xl font-semibold mb-6">{group?.title || 'Skills'}</h3>
          ...
          >
            {(group?.items || []).map((skill, idx) => (
              <div key={idx} className="flex flex-col items-center">
                <img
                  src={skill?.icon || '/assets/icons/default.png'}
                ...
                />
                <p className="mt-2">{skill?.label || 'Unknown'}</p>
              </div>
            ))}
          </div>
        </div>
